...
**The database is `node:sqlite`, not `better-sqlite3`.** `DatabaseSync` is built into
Node 24 and there is no package to install: `better-sqlite3` is absent from
`node_modules`, appears in no source file, and never was a dependency. Worth stating
...
copying `.db`, `-wal` and `-shm` together on 23 Aug produced a snapshot with **no
`creative_ideas` table at all** while the live database had eight rows in it. Use
`VACUUM INTO`, which reads through the log and does not modify the source:

```js
const { DatabaseSync } = require('node:sqlite');
const db = new DatabaseSync('data/dashboard.db', { readOnly: true });
db.exec("VACUUM INTO '" + absoluteOutPath + "'");     // forward slashes
```

**That failure was loud only by luck, and the quiet version is the one to fear.** A
missing table throws. A table that merely *lags a few minutes* returns perfectly
plausible rows, and a migration "verified" against those has been verified against
nothing in particular. Expect stale data, not an error.

**And a snapshot settles only what the data can settle.** It proved a migration and its
backfill for me and could not touch the route guard shipped in the same change, because
a guard is only exercised by a live request — which is how that guard reached a restart
sitting in the wrong route entirely.

<!--COUNTS-->
<!-- Generated by tools/stamp-claude-md.cjs. Do not hand-edit: run the tool. Last: 2026-08-23 -->

| Counted | |
| --- | --- |
| Route files | 65 |
| Endpoints | 286 |
| Panels in the registry | 63 |
| Migrated modules | 35 |
| Tables | 86 |
| Commits | 279, remote: none |
| Panels defining their own api() | 20 |
| Memory store | 225 files |

**Migrated modules**, from `schema_meta`: alerts · analytics · atlas · board · brain · briefing · browsing · budget · cash · creative · crm · drive · exercise · finance · gate · goals · health · inbox · income · income-attribution · income-balances · inventory · journal · lifestyle · mail · runlog · safety · schedule · sessions · tasks · team · todo · viability · wellbeing · work

**Tables**, from `sqlite_master`: alert_events · alert_kinds · analytics_probes · analytics_traffic · atlas_countries · board_imports · board_items · brain_decisions · brain_flags · brain_notes · briefings · browsing_domain_days · browsing_domains · browsing_news_briefings · browsing_news_feedback · browsing_news_topics · budget_lines · cash_counts · creative_developments · creative_ideas · crm_clients · crm_followups · data_access_log · drive_files · drive_sync · exercise_sessions · finance_accounts · finance_assets · finance_rules · finance_transactions · focus_active_sessions · focus_project_targets · focus_sessions · gate_attempts · gate_devices · gmail_messages · gmail_sender_rules · gmail_senders · gmail_sync · goal_steps · goals · health_metrics · inbox_messages · income_balances · income_entries · income_stream_payers · income_streams · inventory_items · journal_entries · lifestyle_chores · lifestyle_done · lifestyle_foods · lifestyle_intake · lifestyle_meals · lifestyle_targets · safety_decisions · safety_limits · safety_payees · schedule_events · schema_meta · scribe_capabilities · scribe_caps · scribe_proposals · scribe_runs · sqlite_sequence · tasks · team_arbitrations · team_assignments · team_decisions · team_handovers · team_owner_item_filings · team_owner_items · team_plans · team_responses · team_reviews · team_sessions · team_steering · todo_items · todo_kind_log · todo_notes · tool_runs · viability_scenarios · wellbeing_entries · wellbeing_quiet · wishlist_items · work_items
...
`Enable-ScheduledTask -TaskName GarageServer` restores it.

**`GarageTelemetryHourly` is NOT ours — corrected 18 Aug 2026.** This line used to read
*"must keep running: it writes the ledger the console displays"*, and that stopped being
true when the Garage dropped the four Oxford pages it was borrowing. The task runs Oxford
AutoWorks' own script with Oxford as its working directory. Mission Control parses its own
sessions off the existing briefing pass instead. Nothing here depends on it: leave it alone,
or disable it as part of Oxford.

The workspace `CLAUDE.md` had already recorded this, and **this file went on contradicting
it**, so which answer a session got depended on which file it happened to read. That is the
argument for generating what can be generated and cross-checking what cannot.
...
## The board, and the team — 19 August 2026

Two modules and one standard, from the owner: *"Standardize team structures... I need a
central place to view open bugs, requests for all projects. Please have each session record
write a handover."* **`TEAM.md` is the standard and is short. Read it before messaging the
owner, starting a tracker, or delegating anything.**

**The board (`/api/board`) is the one place work is listed.** It imports each project's own
tracker **read-only** and shows it beside the backlog — `Survive/BUGS.md` and
`dash/requests.jsonl` stay the place to WRITE, so a session logs a bug with no server and no
network. Adding a project means one parser in `server/trackers.js`, and that parser must
report its residue. `todo_items` gained `project` and `kind` (migration 5) to make it
filterable; 44 rows were assigned a project mechanically and 119 left explicitly unassigned,
because a plausible mapping is still a guess.

**The team (`/api/team`) exists to ration one thing: the owner's attention.** Nine sessions
were running when it was built. Roles are therefore defined by what they may **interrupt**:

| | |
|---|---|
| **worker** | hands over every shift; never interrupts. Owner-facing items go under *Blocked on you*. |
| **supervisor** | reads every handover, drafts the plan, delegates once confirmed; never interrupts. |
| **manager** | confirms or returns the plan. **The only role in the chain that may interrupt the owner** — once a day, as the steering block leading the morning briefing. |
| **architect** | outside the chain by the owner's decision. Takes work from him directly. **If a second one appears, collapse the role back in.** |

Only the last step of the chain is mechanically enforceable, and it is enforced:
`POST /api/team/assign` **409s against an unconfirmed plan**. A confirmation with no verdict
and a steering question with no recommendation are both refused — a stamp without reasoning
looks like scrutiny happened, and a question without a recommendation hands the thinking back.
Everything earlier in the chain happens in chat windows, where no schema reaches, so it is
*recorded* rather than enforced and a skipped step is visible afterwards.

Three properties worth keeping, each of which was a bug first:

- **Silence is reported by name.** A session that filed nothing and one with nothing to say
  are otherwise the same empty space, and the second is rare.
- **A `needs_owner` item can be resolved without the owner** — with a required note. The first
  handover ever filed asked a question that events answered twenty minutes later while it was
  still queued for him. A stale question spending his attention is the exact failure this
  whole structure exists to prevent.
- **A handover is never refused for being incomplete**, and spools to disk if the server is
  down. A session that gets a 400 at the end of its shift does not try again, and its context
  is gone by morning.
...
- **Never start a second bug tracker or backlog.** One board. Add a parser to
  `server/trackers.js` instead — and never let it write back to a file it does not own.
- **Never interrupt the owner unless you are the manager or the architect.** Put it in your
  handover under *Blocked on you*; it reaches him through the daily steering quiz. A P0 —
  active data loss, a live security hole, a defect invalidating the work in flight — still
  drops everything, as it always has.
