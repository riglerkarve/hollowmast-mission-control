...
// The agent-exclusion filter is IMPORTED, never retyped. focus_sessions is read in eight
// places here, and every known model actor must stay out of the owner's streaks and totals.
...
const { NOT_AGENT } = require('./sessions');
  ...
  const todayRow = db
    .prepare(
      `SELECT COUNT(*) AS c FROM focus_sessions
         WHERE kind = 'work' AND ${NOT_AGENT} AND date(completed_at) = date('now', 'localtime')`
    )
    .get();
  ...
  const row = db
    .prepare(
      `SELECT COUNT(*) AS sessions, COALESCE(SUM(duration_minutes), 0) AS minutes, MIN(date(completed_at)) AS since
       FROM focus_sessions WHERE kind = 'work' AND ${NOT_AGENT}`
    )
    .get();
...
function standing() {
  ...
  const focus = q(`SELECT COUNT(*) c, COALESCE(SUM(duration_minutes),0) m FROM focus_sessions WHERE ${NOT_AGENT}`);
  ...
  return {
    ...
    streaks: [
      ...
      { label: 'days with a focus session', ...streak('focus_sessions', 'completed_at', NOT_AGENT) },
    ],
