...
router.post('/entries', (req, res) => {
  ...
  if (m === null && !String(note || '').trim()) {
    return res.status(400).json({ error: 'an entry needs a mood, a note, or both' });
  }

  const info = db.prepare('INSERT INTO wellbeing_entries (date, mood, note) VALUES (?, ?, ?)')
